...
### 5.5.1 “玻璃面板浮层”规范（叠在图片底部的信息条）
> 目标：卡片视觉以**图片为主**，文字信息以“渐进披露”的方式出现；默认信息条像一块半透明玻璃压在图片底部，既能读清文字，又不会抢走图片注意力。

#### 布局与层级
- **叠加方式**：`.cat-info` 通过负外边距叠到图片底部（推荐 `margin-top: -52px ~ -64px`）
- **内收**：左右内收 `10px`（避免贴边显得廉价），底部留 `12px` 呼吸位
- **圆角**：与卡片保持一致的高级感，推荐 `14px`
- **层级**：`.cat-info` `z-index` 高于图片控件与阴影（建议 `z-index: 25`），保证文字始终可读

#### 玻璃材质（关键参数，必须同时具备）
- **背景透明渐变**（比纯透明更“像玻璃”）：
  - 建议：`linear-gradient(180deg, rgba(255,255,255,0.62) 0%, rgba(255,255,255,0.86) 100%)`
- **毛玻璃模糊**：
  - `backdrop-filter: blur(12px)`
  - 必须同时加：`-webkit-backdrop-filter: blur(12px)`（否则部分环境看不到 blur）
- **边框**：1px `rgba(0,0,0,0.08)`（不要厚边框）
- **阴影**：轻但可感知（强调“浮在图片上”而不是“内容区”）
  - 建议：`0 10px 30px rgba(0,0,0,0.10)`

#### 文本可读性（必须保证）
- **对比度**：面板上正文与背景对比 ≥ 4.5:1
- **密度**：默认显示“名字 + 品种 + 2 行描述”
  - `.cat-name` 推荐 16–18px、字重 650–700
  - `.cat-breed` 12–13px，弱化色
  - `.cat-description` 默认 2 行；hover 可扩到 4 行（桌面端）
- **避免压迫图片**：信息条高度尽量控制在 2–3 行文本可读的范围

#### 与图片区控件的避让规则
- 轮播指示器 `.carousel-indicator` 建议上移，避免被玻璃面板压住：
  - 推荐：`bottom: 64px`（根据实际面板高度微调）
- 图片角落的图标按钮（如 `.gif-button`）应优先置顶（靠近图片上沿），避免与信息条拥挤

#### 状态规则（重要例外）
- **编辑态**：当卡片进入 `.editing` 时，玻璃面板必须“退场”恢复普通内容区，避免透明/叠层影响表单可用性：
  - `.cat-card.editing .cat-info { margin: 0; background: #fff; backdrop-filter: none; border: none; box-shadow: none; }`
- **移动端**：无 hover，建议保留主要信息可见；面板的叠加高度不要太大（可把负 margin 稍微减小）
